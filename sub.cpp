...
// các phép toán chính
    ...
    bignum operator/(bignum in)
    {
        bignum a = check(*this), b = check(in), wait, ou;
        if (a.digit.size() < b.digit.size())
            return tobignum(0, false);
        ou.digit.clear(); wait.digit.clear();
        b.negative = wait.negative = false;

        while (a.digit.size())
        {
            if (wait > b)
            {
                long long tempW = toll(wait), tempB = toll(b);
                ou.digit.insert(ou.digit.begin(), tempW / tempB);
                wait = tobignum(tempW % tempB, false);
            }
            wait.digit.insert(wait.digit.begin(), *(a.digit.end() - 1));
            a.digit.pop_back();
        }
        ou.digit.insert(ou.digit.begin(), toll(wait) / toll(b));
